...
bool AchievementsSubsystem::Init() {
    Logger::Info("AchievementsSubsystem: Initializing...");

    // Get and validate platform
    auto platform = Get<IPlatformSubsystem>();
    if (!platform || !platform->IsOnline()) {
        Logger::Error("AchievementsSubsystem: Platform not available or offline");
        return false;
    }

    EOS_HPlatform platform_handle = platform->GetPlatformHandle();
    if (!platform_handle) {
        Logger::Error("AchievementsSubsystem: Invalid platform handle");
        return false;
    }

    achievements_handle = EOS_Platform_GetAchievementsInterface(platform_handle);
    if (!achievements_handle) {
        Logger::Error("AchievementsSubsystem: Failed to get achievements interface");
        return false;
    }

    stats_handle = EOS_Platform_GetStatsInterface(platform_handle);
    if (!stats_handle) {
        Logger::Error("AchievementsSubsystem: Failed to get stats interface");
        return false;
    }

    setup_notifications();
    Logger::Info("AchievementsSubsystem: Initialized successfully");
    return true;
}
...
void AchievementsSubsystem::Shutdown() {
    ...
    Logger::Info("AchievementsSubsystem: Shutdown complete");
}

bool AchievementsSubsystem::QueryAchievementDefinitions() {
    if (!achievements_handle) {
        Logger::Warning("AchievementsSubsystem: Not initialized");
        return false;
    }
    ...
    if (!auth || !auth->IsLoggedIn()) {
        Logger::Warning("AchievementsSubsystem: User not authenticated");
        return false;
    }

    EOS_ProductUserId product_user_id = auth->GetProductUserId();
    if (!EOS_ProductUserId_IsValid(product_user_id)) {
        Logger::Error("AchievementsSubsystem: Invalid Product User ID");
        return false;
    }
...
bool AchievementsSubsystem::UnlockAchievements(const Array& achievement_ids) {
    ...
    if (achievement_ids.size() == 0) {
        Logger::Warning("AchievementsSubsystem: No achievement IDs provided");
        return false;
    }
    ...
    EOS_ProductUserId product_user_id = auth->GetProductUserId();
    if (!EOS_ProductUserId_IsValid(product_user_id)) {
        Logger::Warning("AchievementsSubsystem: Invalid Product User ID");
        return false;
    }
    ...
    context.release();
    Logger::Info("AchievementsSubsystem: Starting achievement unlock operation");
    return true;
}
...
bool AchievementsSubsystem::IngestStat(const String& stat_name, int amount) {
    ...
    if (stat_name.is_empty()) {
        Logger::Warning("AchievementsSubsystem: Stat name is empty");
        return false;
    }

    if (amount <= 0) {
        Logger::Warning("AchievementsSubsystem: Amount must be positive");
        return false;
    }
...
bool AchievementsSubsystem::QueryStats() {
    if (!stats_handle) {
        Logger::Error("AchievementsSubsystem: Not initialized");
        return false;
    }
    ...
    auto auth = Get<IAuthenticationSubsystem>();
    if (!auth || !auth->IsLoggedIn()) {
        Logger::Error("AchievementsSubsystem: User not authenticated");
        return false;
    }
...
void AchievementsSubsystem::setup_notifications() {
    ...
    if (unlock_notification_id == EOS_INVALID_NOTIFICATIONID) {
        Logger::Error("AchievementsSubsystem: Failed to register unlock notifications");
    }
}
...
void EOS_CALL AchievementsSubsystem::on_query_definitions_complete(const EOS_Achievements_OnQueryDefinitionsCompleteCallbackInfo* data) {
    ...
    if (data->ResultCode == EOS_EResult::EOS_Success) {
        ...
        Logger::Error("AchievementsSubsystem: Achievement definitions query failed");

        // Call the callback with failure
        if (self->achievement_definitions_callback.is_valid()) {
            Array empty_definitions;
            Logger::Info("AchievementsSubsystem: Calling achievement definitions callback with failure (empty definitions)");
...
void EOS_CALL AchievementsSubsystem::on_query_player_achievements_complete(const EOS_Achievements_OnQueryPlayerAchievementsCompleteCallbackInfo* data) {
    ...
    if (data->ResultCode == EOS_EResult::EOS_Success) {
        ...
        if (!auth || !auth->IsLoggedIn()) {
            Logger::Error("AchievementsSubsystem: User not authenticated during player achievements query callback");
            return;
        }

        EOS_ProductUserId product_user_id = auth->GetProductUserId();
        if (!EOS_ProductUserId_IsValid(product_user_id)) {
            Logger::Error("AchievementsSubsystem: Invalid Product User ID during player achievements query callback");
            return;
        }
        ...
        Logger::Error("AchievementsSubsystem: Player achievements query failed");

        // Call the callback with failure
        if (self->player_achievements_callback.is_valid()) {
            Array empty_achievements;
            Logger::Info("AchievementsSubsystem: Calling player achievements callback with failure (empty achievements)");
...
void EOS_CALL AchievementsSubsystem::on_unlock_achievements_complete(const EOS_Achievements_OnUnlockAchievementsCompleteCallbackInfo* data) {
    ...
    if (data->ResultCode == EOS_EResult::EOS_Success) {
        ...
        Logger::Error("AchievementsSubsystem: Achievement unlock failed");
...
void EOS_CALL AchievementsSubsystem::on_achievements_unlocked(const EOS_Achievements_OnAchievementsUnlockedCallbackV2Info* data) {
    ...
    Logger::Info(vformat("AchievementsSubsystem: Achievement unlocked: %s", achievement_id));
}

void EOS_CALL AchievementsSubsystem::on_ingest_stat_complete(const EOS_Stats_IngestStatCompleteCallbackInfo* data) {
    if (!data || !data->ClientData) {
        Logger::Error("AchievementsSubsystem: Invalid callback data");
        return;
    }
    ...
    if (data->ResultCode == EOS_EResult::EOS_Success) {
        ...
        Logger::Error(vformat("AchievementsSubsystem: Failed to ingest stat, error: %d", (int64_t)data->ResultCode));
    }
}
...
void EOS_CALL AchievementsSubsystem::on_query_stats_complete(const EOS_Stats_OnQueryStatsCompleteCallbackInfo* data) {
    ...
    if (data->ResultCode == EOS_EResult::EOS_Success) {
        ...
        Logger::Error(vformat("AchievementsSubsystem: Failed to query stats, error: %d", (int64_t)data->ResultCode));
